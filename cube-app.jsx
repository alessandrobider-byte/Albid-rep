      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "8px", flexShrink: 0, alignSelf: "flex-start" }}>
        ...
        {isDFC && (
          <div onClick={() => setFlipped(f => !f)} style={{
            ...
            color: "#fff", fontSize: "12px", fontWeight: "400",
            letterSpacing: "0.08em", textTransform: "uppercase",
            width: "150px", boxSizing: "border-box",
          }}>
            <span style={{ fontSize: "14px" }}>⇄</span> Transform
          </div>
        )}
      </div>
...
const PREDEFINED_MAIN_ARCHETYPES = [
  ...
  { name:"Legends", desc:"Payoffs that reward controlling legendary permanents — lords, enablers, and synergy pieces that grow stronger with each legend you control. Requires support cards that trigger on or reward legendary status.", ratio:"12%" },
  { name:"Gain Control", desc:"Permanents and spells that steal control of opposing creatures or permanents. Requires a critical mass of control effects to constitute a reliable strategy.", ratio:"0%" },
  { name:"Auras Matter", desc:"Distinct from Enchantress: focuses on buffing creatures with auras rather than generating card advantage. Payoffs reward enchanting your own creatures — bestow, totem armor, and aura synergy pieces.", ratio:"12%" },
  { name:"Equipments", desc:"Artifacts that attach to creatures to grant persistent bonuses. Requires both equipment cards and payoff creatures that benefit from being equipped — tutors, cost reducers, and creatures with equipment synergies.", ratio:"12%" },
];
...
function ArchetypesAnalysisWrapper({ cards, db }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:"1px", borderBottom:"1px solid #222", backgroundColor:"#0d0d0d", paddingLeft:"24px" }}>
        {[["main","Main Archetypes"],["tribal","Tribal Archetypes"],["support","Support"]].map(([t,label]) => (
          <div key={t} onClick={() => setArchTab(t)} style={archTab===t
            ? { padding:"8px 20px", cursor:"pointer", fontSize:"13px", color:"#fff", borderBottom:"2px solid #d4af37", fontWeight:"600" }
            : { padding:"8px 20px", cursor:"pointer", fontSize:"13px", color:"#aaa", borderBottom:"2px solid transparent" }}>
            {label}
          </div>
        ))}
      </div>
...
function TribalAnalysisPage({ cards, db }) {
  ...
  // Per-guild top tribal archetypes
  const guildMatrix = GUILDS_LIST.map(g => {
    const ranked = allData
      .map(a => {
        const gd = a.guildData.find(x => x.guild === g.name);
  ...
  return (
    <div style={{ ...S.page, maxWidth:"960px" }}>
      {/* Box 1: Tribal per guild */}
      <div style={S.box}>
        <div style={S.boxTitle}>Tribal archetypes per guild</div>
        <div style={{ overflowX:"auto" }}>
          <table style={{ borderCollapse:"collapse", width:"100%" }}>
            <thead>
              <tr>
...
const UTILITY_DATA = [
    ...
    { cat:"Board Presence", type:"Blink",       sub:"Blink",            colors:"WU",   desc:"Exile a permanent and return it immediately — resets summoning sickness, removes negative auras or counters, and most importantly retriggers enter-the-battlefield effects. Examples: Momentary Blink, Ephemerate, Ghostly Flicker, Restoration Angel, Conjurer's Closet.", core:0, targets:{pSelf:0,pOpp:0,creature:1,artifact:1,enchant:1,pw:0,land:0,token:0,battle:0,instant:0,sorcery:0} },
    ...
    { cat:"Removal", type:"Gain Control",      sub:"Gain Control",      colors:"U",    desc:"Spells and permanents that permanently or temporarily steal control of an opponent's creature or permanent. Bypasses indestructible and hexproof. Examples: Control Magic, Treachery, Act of Treason, Bribery, Mind Control.", core:0, targets:{pSelf:0,pOpp:0,creature:1,artifact:1,enchant:1,pw:1,land:0,token:1,battle:0,instant:0,sorcery:0} },
...
function App() {
  ...
  return (
    ...
    <div style={{ ...S.app, maxWidth:"1024px", margin:"0 auto", minHeight:"100vh", borderLeft:"1px solid #111", borderRight:"1px solid #111" }}>
      ...
      {active === "Analyze" && analysisTab === "Archetypes"  && <ArchetypesAnalysisWrapper cards={cards.filter(c => !c.tags?.maybe_board)} db={db} />}
