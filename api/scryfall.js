export default async function handler(req, res) {
  ...
  try {
    // If set code provided, use /cards/:set/:name endpoint for exact edition
    const url = set
    ...
    if (!response.ok || data.object === "error") {
      // If set-specific lookup fails, try without set
      if (set) {
        const fallback = await fetch(`https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(name)}`);
